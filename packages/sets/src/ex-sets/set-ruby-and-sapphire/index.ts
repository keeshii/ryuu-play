import { Card } from '@ptcg/common';
import { Aggron } from './aggron';
import { Aron } from './aron';
import { Aron2 } from './aron-2';
import { Aron3 } from './aron-3';
import { Beautifly } from './beautifly';
import { Blaziken } from './blaziken';
import { Blaziken2 } from './blaziken-2';
import { Breloom } from './breloom';
import { Camerupt } from './camerupt';
import { Carvanha } from './carvanha';
import { Cascoon } from './cascoon';
import { ChanseyEx } from './chansey-ex';
import { Combusken } from './combusken';
import { Combusken2 } from './combusken-2';
import { DarknessEnergy } from './darkness-energy';
import { Delcatty } from './delcatty';
import { Delcatty2 } from './delcatty-2';
import { Donphan } from './donphan';
import { Dustox } from './dustox';
import { ElectabuzzEx } from './electabuzz-ex';
import { Electrike } from './electrike';
import { Electrike2 } from './electrike-2';
import { Electrike3 } from './electrike-3';
import { EnergyRemoval2 } from './energy-removal-2';
import { EnergyRestore } from './energy-restore';
import { EnergySearch } from './energy-search';
import { EnergySwitch } from './energy-switch';
import { FightingEnergy } from './fighting-energy';
import { FireEnergy } from './fire-energy';
import { Gardevoir } from './gardevoir';
import { Goldeen } from './goldeen';
import { GrassEnergy } from './grass-energy';
import { Grovyle } from './grovyle';
import { Grovyle2 } from './grovyle-2';
import { Hariyama } from './hariyama';
import { Hariyama2 } from './hariyama-2';
import { HitmonchanEx } from './hitmonchan-ex';
import { Kirlia } from './kirlia';
import { Kirlia2 } from './kirlia-2';
import { Koffing } from './koffing';
import { LadyOuting } from './lady-outing';
import { Lairon } from './lairon';
import { Lairon2 } from './lairon-2';
import { LaprasEx } from './lapras-ex';
import { LightningEnergy } from './lightning-energy';
import { Linoone } from './linoone';
import { LumBerry } from './lum-berry';
import { MagmarEx } from './magmar-ex';
import { Makuhita } from './makuhita';
import { Makuhita2 } from './makuhita-2';
import { Makuhita3 } from './makuhita-3';
import { Manectric } from './manectric';
import { Manectric2 } from './manectric-2';
import { Marshtomp } from './marshtomp';
import { Marshtomp2 } from './marshtomp-2';
import { MetalEnergy } from './metal-energy';
import { MewtwoEx } from './mewtwo-ex';
import { Mightyena } from './mightyena';
import { Mightyena2 } from './mightyena-2';
import { Mudkip } from './mudkip';
import { Mudkip2 } from './mudkip-2';
import { Nosepass } from './nosepass';
import { Numel } from './numel';
import { OranBerry } from './oran-berry';
import { Pelipper } from './pelipper';
import { Phanpy } from './phanpy';
import { PokéBall } from './pok-ball';
import { PokémonReversal } from './pokmon-reversal';
import { Pokénav } from './poknav';
import { Poochyena } from './poochyena';
import { Poochyena2 } from './poochyena-2';
import { Poochyena3 } from './poochyena-3';
import { Potion } from './potion';
import { ProfessorBirch } from './professor-birch';
import { PsychicEnergy } from './psychic-energy';
import { RainbowEnergy } from './rainbow-energy';
import { Ralts } from './ralts';
import { Ralts2 } from './ralts-2';
import { Ralts3 } from './ralts-3';
import { Sceptile } from './sceptile';
import { Sceptile2 } from './sceptile-2';
import { ScytherEx } from './scyther-ex';
import { Seaking } from './seaking';
import { Sharpedo } from './sharpedo';
import { Shroomish } from './shroomish';
import { Silcoon } from './silcoon';
import { Skitty } from './skitty';
import { Skitty2 } from './skitty-2';
import { Skitty3 } from './skitty-3';
import { Slaking } from './slaking';
import { Slakoth } from './slakoth';
import { SneaselEx } from './sneasel-ex';
import { Swampert } from './swampert';
import { Swampert2 } from './swampert-2';
import { Swellow } from './swellow';
import { Switch } from './switch';
import { Taillow } from './taillow';
import { Torchic } from './torchic';
import { Torchic2 } from './torchic-2';
import { Treecko } from './treecko';
import { Treecko2 } from './treecko-2';
import { Vigoroth } from './vigoroth';
import { Wailmer } from './wailmer';
import { Wailord } from './wailord';
import { WaterEnergy } from './water-energy';
import { Weezing } from './weezing';
import { Wingull } from './wingull';
import { Wurmple } from './wurmple';
import { Zigzagoon } from './zigzagoon';

export const setRubyAndSapphire: Card[] = [
  new Aggron(),
  new Aron(),
  new Aron2(),
  new Aron3(),
  new Beautifly(),
  new Blaziken(),
  new Blaziken2(),
  new Breloom(),
  new Camerupt(),
  new Carvanha(),
  new Cascoon(),
  new ChanseyEx(),
  new Combusken(),
  new Combusken2(),
  new DarknessEnergy(),
  new Delcatty(),
  new Delcatty2(),
  new Donphan(),
  new Dustox(),
  new ElectabuzzEx(),
  new Electrike(),
  new Electrike2(),
  new Electrike3(),
  new EnergyRemoval2(),
  new EnergyRestore(),
  new EnergySearch(),
  new EnergySwitch(),
  new FightingEnergy(),
  new FireEnergy(),
  new Gardevoir(),
  new Goldeen(),
  new GrassEnergy(),
  new Grovyle(),
  new Grovyle2(),
  new Hariyama(),
  new Hariyama2(),
  new HitmonchanEx(),
  new Kirlia(),
  new Kirlia2(),
  new Koffing(),
  new LadyOuting(),
  new Lairon(),
  new Lairon2(),
  new LaprasEx(),
  new LightningEnergy(),
  new Linoone(),
  new LumBerry(),
  new MagmarEx(),
  new Makuhita(),
  new Makuhita2(),
  new Makuhita3(),
  new Manectric(),
  new Manectric2(),
  new Marshtomp(),
  new Marshtomp2(),
  new MetalEnergy(),
  new MewtwoEx(),
  new Mightyena(),
  new Mightyena2(),
  new Mudkip(),
  new Mudkip2(),
  new Nosepass(),
  new Numel(),
  new OranBerry(),
  new Pelipper(),
  new Phanpy(),
  new PokéBall(),
  new PokémonReversal(),
  new Pokénav(),
  new Poochyena(),
  new Poochyena2(),
  new Poochyena3(),
  new Potion(),
  new ProfessorBirch(),
  new PsychicEnergy(),
  new RainbowEnergy(),
  new Ralts(),
  new Ralts2(),
  new Ralts3(),
  new Sceptile(),
  new Sceptile2(),
  new ScytherEx(),
  new Seaking(),
  new Sharpedo(),
  new Shroomish(),
  new Silcoon(),
  new Skitty(),
  new Skitty2(),
  new Skitty3(),
  new Slaking(),
  new Slakoth(),
  new SneaselEx(),
  new Swampert(),
  new Swampert2(),
  new Swellow(),
  new Switch(),
  new Taillow(),
  new Torchic(),
  new Torchic2(),
  new Treecko(),
  new Treecko2(),
  new Vigoroth(),
  new Wailmer(),
  new Wailord(),
  new WaterEnergy(),
  new Weezing(),
  new Wingull(),
  new Wurmple(),
  new Zigzagoon(),
];
