...
import { MrMimeEx2 } from '../../../src/ex-sets/set-firered-and-leafgreen/mr-mime-ex-2';
...
describe('Mr. Mime ex RG-2', () => {
  let sim: Simulator;

  beforeEach(() => {
    sim = TestUtils.createTestSimulator();

    TestUtils.setActive(sim, [new MrMimeEx2()], [CardType.PSYCHIC, CardType.COLORLESS]);
    TestUtils.setDefending(sim, [new TestPokemonStrong30()]);
  });

  it('Should prevent even damage amounts with Magic Evens', () => {
  ...
  it('Should not prevent odd damage amounts with Magic Evens', () => {
    const { opponent, player } = TestUtils.getAll(sim);
    TestUtils.setActivePlayer(sim, opponent);
  ...
  it('Should not prevent damage when Magic Evens is blocked', () => {
    const { opponent, player } = TestUtils.getAll(sim);
  ...
  it('Should not prevent damage when Mr. Mime has evolved', () => {
    const { opponent, player } = TestUtils.getAll(sim);
    player.active.pokemons.cards = [new MrMimeEx2(), new TestPokemon()];
    player.bench[0] = TestUtils.pokemonSlot([new TestPokemonBlockPowers()]);
    TestUtils.setActivePlayer(sim, opponent);
  ...
  it('Should use Breakdown to place damage counters equal to opponent hand size', () => {
    ...
    opponent.hand.cards = [new TestPokemon(), new TestPokemon(), new TestPokemon(), new TestPokemon()];
